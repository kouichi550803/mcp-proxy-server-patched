  ...
  try {
    if (isSSEConfig(transportConfig)) {
      ...
      if (customHeaders) {
       ...
       logger.error(`Invalid or unknown transport type in configuration for server: ${name}`);
     }
   } catch (error) {
     ...
     logger.error(`Failed to create transport ${transportType} to ${name}:`, error);
   }

   if (!transport || !transportType) {
     logger.warn(`Transport or transportType for ${name} not available.`);
...
export const createClients = async (mcpServers: Record<string, TransportConfig>): Promise<ConnectedClient[]> => {
  ...
  for (const [name, transportConfig] of Object.entries(mcpServers)) {
    ...
    while (retry) {
      ...
      const { client, transport, transportType } = createClient(name, transportConfig);
      if (!client || !transport || !transportType) {
        logger.warn(`Skipping client ${name} due to failed client/transport creation.`);
        break;
      }
      ...
      try {
        ...
        if (retry) {
          ...
          logger.log(`Retry connection to ${name} in ${waitFor}ms (${count}/${retries})`);
          await sleep(waitFor);
        }
      }
...
export async function reconnectSingleClient(
...
): Promise<Omit<ConnectedClient, 'name'>> {
  ...
  if (existingCleanup) {
    try {
      ...
      logger.warn(`Error during cleanup of existing client ${name} before reconnect: ${e.message}`);
    }
  }
  ...
  try {
    ...
    logger.error(`Failed to create transport for ${name} during reconnect: ${error.message}`);
    throw error;
  }

  if (!transport || !determinedTransportType) {
  ...
  try {
    ...
    return {
      client: newSdkClient,
      config: transportConfig,
      transportType: determinedTransportType,
      cleanup: async () => {
        if (finalTransport) {
    ...
    logger.error(`Failed to connect to ${name} during reconnect attempt: ${error.message}`);
    try {
      if (transport) {
          await transport.close();
      }
    } catch (closeError: any) {
      logger.warn(`Failed to close transport for ${name} after reconnect failure: ${closeError.message}`);
    }
    throw error;
  }
}
